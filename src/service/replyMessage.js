  ...
  message = ({ messageType, profile, user, data = {} }) => {
    switch (messageType) {
          ...
          contents: {
            ...
            "body": {
              ...
              "contents": [
                ...
                {
                  "type": "text",
                  "text": "♠️ขอให้เพลิดเพลินกับการเล่นนะคะ♠️",
                  "wrap": true
                }
              ]
            }
            ...
            contents: {
              ...
              body: {
                ...
                contents: [
                  {
                    type: 'text',
                    text: '♠️ กรณีเปลี่ยนเจ้า / เปลี่ยนไพ่ ♠️',
                    align: 'center'
                  },
                  {
                    type: 'text',
      ...
      case "RESET_ROUND_RESULT": {
        return {
          type: "text",
          text: `ทำการรีเซ็ตผลลัพธ์รอบที่ ${data?.id} กรุณาใส่ผลลัพธ์ใหม่อีกครั้งค่ะ ‼️`,
        };
      }
      case "NO_ROUND": {
        return {
          type: "text",
          text: `‼️ ขณะนี้ไม่มีรอบการเล่นที่เปิดอยู่ กรุณารอเจ้ามือเปิดรอบค่ะ ‼️`,
        };
      }
      case "EXISTS_ROUND": {
        return {
          type: "text",
          text: `⭕️ ขณะนี้อยู่ในรอบที่ ${data?.roundId} ⭕️`,
        };
      }
      case "WAITING_ROUND_RESULT": {
        return {
          type: "text",
          text: `⚠️ ขณะนี้มีรอบที่กำลังรอผลลัพธ์อยู่ รอบที่ ${data?.roundId} ⚠️`,
        };
      }
      case "OPEN_ROUND": {
        return [
          {
            type: "text",
            text: `♣️♦️♥️♠️ รอบที่ ${data?.roundId} ♣️♦️♥️♠️`,
          },
          {
            type: "image",
            originalContentUrl:
...
📍คนตัดไพ่ห้ามแทงเจ้ามือ`
      ...
      case "INSUFFICIENT_BALANCE": {
        return {
          type: "text",
          text: `⚠️ ยอดเงินของคุณไม่เพียงพอ ⚠️ เครดิตปัจจุบัน ${user?.wallet?.balance}`,
        };
        break;
      }
      case "ADD_CREDIT": {
      ...
      case "NO_ROUND_ADMIN": {
        return {
          type: "text",
          text: `❌⭕️ ขณะนี้ไม่มีรอบที่เปิดอยู่ ⭕️❌`,
        };
        break;
      }
